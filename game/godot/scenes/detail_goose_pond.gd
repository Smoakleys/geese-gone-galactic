...
func build(root: Node3D) -> void:
    ...
    # goose body sphere
    var body_pos = Vector3(0, 0.9, -2)
    add_sphere(root, 0.9, Color.WHITE, body_pos)
    # neck spheres
    var neck1_pos = body_pos + Vector3(0.4, 0.5, -0.2)
    add_sphere(root, 0.3, Color.WHITE, neck1_pos)
    var neck2_pos = neck1_pos + Vector3(0.2, 0.4, -0.1)
    add_sphere(root, 0.3, Color.WHITE, neck2_pos)
    # head sphere
    var head_pos = neck2_pos + Vector3(0.15, 0.35, -0.05)
    add_sphere(root, 0.4, Color.WHITE, head_pos)
    # beak box at front of head
    var beak_pos = head_pos + Vector3(-0.5, 0, -0.2)
    add_box(root, Vector3(0.2, 0.1, 0.05), Color(1, 0.5, 0), beak_pos)
    # tail box behind body
    var tail_pos = body_pos + Vector3(-0.6, 0.2, 0.1)
    add_box(root, Vector3(0.5, 0.3, 0.4), Color.WHITE, tail_pos)
