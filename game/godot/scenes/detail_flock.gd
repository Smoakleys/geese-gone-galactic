...
func build(root: Node3D) -> void:
    ...
    # First goose
    var body_pos = Vector3(-4, 0, -2)
    add_sphere(root, 0.7, Color.WHITE, body_pos)
    var neck1_pos = body_pos + Vector3(0, 0.4, -0.5)
    add_sphere(root, 0.25, Color.WHITE, neck1_pos)
    var neck2_pos = body_pos + Vector3(0, 0.6, -1)
    add_sphere(root, 0.25, Color.WHITE, neck2_pos)
    var head_pos = body_pos + Vector3(0, 0.8, -1.2)
    add_sphere(root, 0.35, Color.WHITE, head_pos)
    var beak_size = Vector3(0.2, 0.1, 0.4)
    var beak_pos = head_pos + Vector3(0, 0, -0.5)
    add_box(root, beak_size, Color(1, 0.5, 0), beak_pos)

    # Second goose
    body_pos = Vector3(4, 0, -2)
    add_sphere(root, 0.7, Color.WHITE, body_pos)
    neck1_pos = body_pos + Vector3(0, 0.4, -0.5)
    add_sphere(root, 0.25, Color.WHITE, neck1_pos)
    neck2_pos = body_pos + Vector3(0, 0.6, -1)
    add_sphere(root, 0.25, Color.WHITE, neck2_pos)
    head_pos = body_pos + Vector3(0, 0.8, -1.2)
    add_sphere(root, 0.35, Color.WHITE, head_pos)
    beak_pos = head_pos + Vector3(0, 0, -0.5)
    add_box(root, beak_size, Color(1, 0.5, 0), beak_pos)
