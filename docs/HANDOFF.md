...
- **(stale) VISUALS: NOW ON GEMINI + a LOCKED VIEWPOINT — BLOCKED on a free key (PRs #391-396).** Bridger
  ...
  PLAN per Bridger: use **Google Gemini (Nano Banana) via the FREE AI Studio key** with VERY strict prompts.
  Built + turnkey: `ops/gemini_art.py` — every asset prompt appends ONE locked viewpoint (3/4 iso, 30 deg,
  orthographic, light top-left, white bg), cuts the bg to transparent (composite-ready), a `--reference`
  real-game screenshot to grade against, retry/backoff. **BLOCKED: needs the free key** at
  `ops/gemini_key.local.txt` (gitignored) — `aistudio.google.com/apikey`, no billing. When it lands:
  generate under the locked viewpoint, GRADE against the reference on a strict rubric, regenerate until
  viewpoint/scale/spacing hold, then lay out with breathing room. **READ [[visual-review-discipline]]**
  (I badly over-praise my own visuals — default harsh, compare to a reference, never judge blind). The old
  Pollinations `ops/generate_art.py` + `game/art_view.py` composite still exist but produced the rejected
  look; do NOT hand-tune them blind. See [[ggg-visuals-priority]].
